...
$modalList.on(closeModal, (modals, id) =>
    modals.filter((modal: { id: void }) => modal.id !== id)
);
...
sample({
    ...
    target: $modalIsOpen,
    fn: (value, target) =>
        value.find((value: { id: void }) => value.id === target),
});
...
sample({
    ...
    target: $modalIsOpen,
    fn: (value, target) =>
        value.filter((value: { id: void }) => value.id !== target),
});
